	...
	<head>
		...
		<script type="text/javascript">
			...
			function getApprovalStatus(role){
				...
				$.ajax({
					type:"post",
					url:"http://123.56.68.20:8080/Batik/approval/getUnapproval",
					data : data,
					dataType: "json",
			...
			function showDisableButton(){
				...
				html = '';
				html+= '<button id="goods_print" style="margin-left: 500px;width:100px;height:35px;background-color:#263236;color: #F1F1F1;" disabled="disabled">审批通过</button>'
						 +  '<button id="goods_output" style="margin-left: 30px;width:100px;height:35px;background-color:#209E85;color: #F1F1F1;" disabled="disabled">审批退回</button>'
						 +  '<button id="goods_save" style="margin-left: 30px;width:100px;height:35px;background-color:#263236;color: #F1F1F1;">返回</button>';
				$("#approvalButton").html(html);
			}
			...
			function approvalPass(){
				alert("审批通过");
				var purchasePlanCode = $("#purchasePlanCode").text();
				alert(purchasePlanCode)
				var cName = $("#cname").val();
				var explain = document.getElementById("approvalOpinion").value;
				var data = {
					"cName" : cName,
				...
				$.ajax({
					type:"post",
					url:"http://123.56.68.20:8080/Batik/approval/approvalPass",
					data:data,
					dataType:"json",
					success:function(ret){
						console.log(JSON.stringify(ret));
			...
			function approvalBack(){
				...
				var data = {
					"cName" : cName,
					"purchasePlanCode" : purchasePlanCode,
					"approvalOpinion" : "审批退回",
					"explain" : explain
				}
				console.log(JSON.stringify(data))
				$.ajax({
					type:"post",
					url:"http://123.56.68.20:8080/Batik/approval/approvalBack",
					data:data,
					dataType:"json",
					success:function(ret){
						console.log(JSON.stringify(ret));
			...
			<table class="layui-table">
				...
				<tbody id="approvalStatus">
					<tr>
						...
						<td id="purchasePlanCode">xxxxxx</td>
						<td>xxx</td>
						<td>xxx</td>
						<td>xx</td>
						<td>xx</td>
